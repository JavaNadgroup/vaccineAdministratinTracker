 ...
 * @author ogrey
 */
public class EmailResponse extends SimpleTagSupport {
    ...
    public void doTag() throws JspException, IOException {
        ...
        if(email !=""){
            ...
            try{ 
                ...
                String sql = "SELECT * FROM messages WHERE reciever = '"+email+"'";
                ResultSet rs = st.executeQuery(sql);
                if(rs.next()){
                    out.println("<div class='flex items-center justify-center min-h-screen p-5 bg-gray-600 min-w-screen'>");
                    out.println("<div class='max-w-xl p-8 text-center text-gray-800 bg-white shadow-xl lg:max-w-3xl rounded-3xl lg:p-12'>");
                    out.println("<h3 class='text-2xl'><span class='text-3xl font-bold text-blue-500'>Subject: </span><span class='underline font-bold'>Tommorrow is your vaccination date</span></h3>");
                    out.println("<h3 class='text-2xl '><span class='text-3xl font-bold text-blue-500'>From: ");
                    out.println("</span>");
                    out.println(rs.getString("sender"));
                    out.println("</h3>");
                    out.println("<div class='flex justify-center'>");
                    out.println("<svg class='w-32 h-32' viewBox='0 0 50 21' fill='none' xmlns='http://www.w3.org/2000/svg'>");
                    out.println("<path");
                    ...
                    out.println("fill='url(#paint0_linear)' />");
                    out.print("<rect y='5' width='15' height='2' rx='1' fill='#3BB54A' />");
                    out.println("<rect y='11' width='15' height='2' rx='1' fill='#3BB54A' />");
                    out.println("<rect y='8' width='6' height='2' rx='1' fill='#3BB54A' />");
                    out.println("<rect y='15' width='6' height='2' rx='1' fill='#3BB54A' />");
                    out.println("<rect x='8' y='8' width='6' height='2' rx='1' fill='#3BB54A' />");
                    out.println("<rect x='8' y='15' width='6' height='2' rx='1' fill='#3BB54A' />");
                    out.println("<defs>");
                    out.println("<linearGradient id='paint0_linear' x1='16.9996' y1='10.4791' x2='47.0156' y2='10.4791'");
                    out.print("gradientUnits='userSpaceOnUse'>");
                    out.println("<stop stop-color='#009217' />");
                    out.print("<stop offset='1' stop-color='#00FF29' />");
                    out.print("</linearGradient>");
                    out.println("</defs>");
                    out.println("</svg>");
                    out.println("</div>");
                    out.println("<h3 class='text-2xl '><span class='text-2xl font-bold text-blue-500'>Message: </span>");
                    out.println("<p class='text-xl italic'>");
                    out.println(rs.getString("message"));
                    out.println("</p>");
                    out.println("<div class='mt-10'>");
                    out.println("<p class='mt-4 text-sm'>We are hoping to see you at the health center tommorrow and our specialists will be ready to work on you. Keep safe, lets fight covid-19 together.");
                    out.println("<a href='welcome.jsp' class='text-blue-600'>click here to go back to the home page.</a>");
                    out.println("</p>");
                    out.println("</div>");
                    out.println("</div>");
                    out.println("</div>");
                }else{
                    out.println("<div class='ml-10 mt-10'>");
                    out.println("<h1 class='text-3xl font-normal leading-normal mt-0 mb-2 text-purple-800'>You Still have more days before You get the vaccination. Keeping checking for an email confirming your vaccination date.</h1>");
                    out.println("<form action='http://localhost:8080/vaccinationTracker/welcome.jsp' method='post'>");
                    out.println("<button type='submit' class=' bg-purple-500 text-white active:bg-purple-600 font-bold uppercase text-base px-8 py-3 rounded-full shadow-md hover:shadow-lg outline-none focus:outline-none mr-1 mr-1 ease-linear transition-all duration-150'>");
                    out.println("Go back to welcome page");
                    out.println("</button>");
                    out.println("</div>");
                }
